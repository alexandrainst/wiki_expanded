...
install: ## Install dependencies
	@echo "Installing the 'wiki_expanded' project..."
	...
	@echo "Installed the 'wiki_expanded' project! You can now activate your virtual environment with 'source .venv/bin/activate'."
...
add-repo-to-git:
	...
	@if [ "$(shell git remote)" = "" ]; then \
		git remote add origin git@github.com:alexandrainst/wiki_expanded.git; \
	fi
...
publish-docs:  ## Publish documentation to GitHub Pages
	...
	@echo "Updated documentation website: https://alexandrainst.github.io/wiki_expanded"

test:  ## Run tests
	@uv run pytest && uv run readme-cov

docker:  ## Build Docker image and run container
	@docker build -t wiki_expanded .
	@docker run -it --rm wiki_expanded
